...
export default async function TeamPage({
  ...
  const [members, audits, params] = await Promise.all([
    listStaffMembers(session.organizationId),
    listAuditEntries(session.organizationId),
    searchParams,
  ]);
  return (
    <div>
      ...
      <nav className="mt-6 flex gap-4 text-sm font-semibold">
        <Link href="/settings/team" className="text-kuartz-ink underline">
          Team
        </Link>
        <Link
          href="/settings/item-types"
          className="text-kuartz-secondary hover:text-kuartz-ink"
        >
          Item types
        </Link>
        <Link
          href="/settings/consultation-note-sources"
          className="text-kuartz-secondary hover:text-kuartz-ink"
        >
        ...
        <Link
          href="/settings/measurement-requirements"
          className="text-kuartz-secondary hover:text-kuartz-ink"
        >
        ...
        <div>
          ...
          <div className="mt-4 divide-y divide-kuartz-line border-y border-kuartz-line">
            {members.map((member) => (
              <div
                key={member.userId}
                className="grid gap-4 py-5 md:grid-cols-[minmax(0,1fr)_14rem] md:items-center"
              >
                <div>
                ...
                <form action={changeStaffRoleAction} className="flex gap-2">
                  <input
                    type="hidden"
                    name="staffMemberId"
                    value={member.userId}
      ...
      <section className="mt-14">
        <div className="flex items-end justify-between">
          <div>
            <p className="eyebrow">Log History</p>
            <h2 className="section-title mt-2">Recent access activity</h2>
          </div>
        </div>
        <div className="mt-4 divide-y divide-kuartz-line border-y border-kuartz-line">
          {audits.length ? (
            audits.map((entry) => (
              <div
                key={entry.id}
                className="grid gap-1 py-4 text-sm sm:grid-cols-[1fr_auto]"
              >
                <p className="font-medium text-[#262b44]">{entry.summary}</p>
                <p className="text-[#858a96]">
                  {entry.actorName ?? "System"} · {date.format(entry.createdAt)}
                </p>
              </div>
            ))
          ) : (
            <p className="py-8 text-sm text-kuartz-muted">
