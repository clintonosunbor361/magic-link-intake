...
import { changeTeamMemberRoleAction, inviteTeamMemberAction } from "@/app/actions/team";
import { requireStaffSession } from "@/lib/auth/session";
import { canManageTeam } from "@/lib/domain/access-control";
import { listAuditEntries, listTeamMembers } from "@/lib/team/repository";
...
export default async function TeamPage({ searchParams }: { searchParams: Promise<{ error?: string; invited?: string }> }) {
  ...
  const [members, audits, params] = await Promise.all([listTeamMembers(session.organizationId), listAuditEntries(session.organizationId), searchParams]);
  return (
    <div>
      ...
      <section className="mt-9 grid gap-10 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div><h2 className="section-title">Active staff</h2><div className="mt-4 divide-y divide-[#d9d8d1] border-y border-[#d9d8d1]">{members.map((member) => <div key={member.userId} className="grid gap-4 py-5 md:grid-cols-[minmax(0,1fr)_14rem] md:items-center"><div><p className="font-semibold text-[#171b36]">{member.fullName}</p><p className="mt-1 text-sm text-[#767b89]">{member.email}</p></div><form action={changeTeamMemberRoleAction} className="flex gap-2"><input type="hidden" name="memberId" value={member.userId} /><input type="hidden" name="version" value={member.version} /><select className="app-input min-h-10 py-2" name="role" defaultValue={member.role}><option value="super_admin">Super Admin</option><option value="admin_assistant">Admin Assistant</option></select><button className="secondary-button" type="submit">Save</button></form></div>)}</div></div>
        <aside><h2 className="section-title">Invite staff</h2><form action={inviteTeamMemberAction} className="mt-4 space-y-4 border-t border-[#d9d8d1] pt-5"><label className="form-group"><span>Full name</span><input className="app-input" name="fullName" required /></label><label className="form-group"><span>Email address</span><input className="app-input" name="email" type="email" required /></label><label className="form-group"><span>Role</span><select className="app-input" name="role" defaultValue="admin_assistant"><option value="admin_assistant">Admin Assistant</option><option value="super_admin">Super Admin</option></select></label><button className="app-button w-full" type="submit">Send invitation</button></form></aside>
      </section>
