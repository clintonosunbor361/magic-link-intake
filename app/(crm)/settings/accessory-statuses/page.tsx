...
import {
  archiveAccessoryStatusAction,
  createAccessoryStatusAction,
  restoreAccessoryStatusAction,
  setAccessoryStatusCompletedAction,
} from "@/app/actions/accessory-statuses";
...
import { canManageAccessoryStatuses } from "@/lib/domain/access-control";

export default async function AccessoryStatusesPage({
  searchParams,
}: {
  searchParams: Promise<{ error?: string }>;
}) {
  const session = await requireStaffSession();
  if (!canManageAccessoryStatuses(session.role)) redirect("/");

  const [statuses, params] = await Promise.all([
    listAccessoryStatuses(session.organizationId, { includeArchived: true }),
    searchParams,
  ]);
  ...
  return (
    <div>
      ...
      <SettingsNav current="/settings/accessory-statuses" />
      ...
      <section className="mt-9 grid gap-10 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <div>
          ...
          {statuses.length ? (
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-kuartz-muted">
                      {status.archivedAt
                        ? "Archived"
                        : index === 0
                          ? "Starting status for new Accessories"
                          : `Position ${index + 1}`}
                    </p>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    <form action={setAccessoryStatusCompletedAction}>
                      ...
                      <Button
                        ...
                        aria-label={`${status.isCompleted ? "Stop counting" : "Count"} ${status.name} as delivered`}
                      >
                        {status.isCompleted ? "Not delivered" : "Mark delivered"}
                      </Button>
                    </form>
                    <form action={status.archivedAt ? restoreAccessoryStatusAction : archiveAccessoryStatusAction}>
                      <input type="hidden" name="statusId" value={status.id} />
                      <input type="hidden" name="version" value={status.version} />
                      <Button
                        type="submit"
            ...
            <EmptyState
              className="mt-4"
              title="No accessory statuses yet"
              description="Add at least one status, including one that counts as delivered, before sourcing accessories."
            />
          )}
        </div>

        <aside>
          <FormDisclosure title="Statuses" buttonLabel="Add status">
          <form action={createAccessoryStatusAction} className="space-y-4 border-t border-kuartz-line pt-5">
