...
export default async function OverviewPage() {
  ...
  const pipelineRows: PipelineRow[] = [
    ...
    { label: "Approvals", value: pendingApprovals.length, href: "/orders", hint: "client decisions" },
  ...
  const workRows = buildWorkRows({ delayed, pendingApprovals, upcomingLooks, today });
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(16rem,0.78fr)_minmax(0,1.35fr)_minmax(17rem,0.78fr)]" aria-label="Priority work">
        <SpotlightCard followUp={followUps[0]} pendingApproval={pendingApprovals[0]} />
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,0.42fr)]" aria-label="Inbox and approvals">
        ...
        <ApprovalsPanel approvals={pendingApprovals} />
      </section>
    </div>
  );
}
...
function SpotlightCard({
  ...
  return (
    <section className="rounded-[1.45rem] border border-white/85 bg-white/86 p-5 shadow-[0_22px_65px_rgba(21,22,63,0.08)] backdrop-blur-xl sm:p-6">
      ...
      <h2 className="mt-5 text-lg font-extrabold text-kuartz-ink">{pendingApproval?.clientName}</h2>
      <p className="mt-1 text-sm text-kuartz-muted">{pendingApproval?.orderTitle}</p>
      <p className="mt-5 rounded-[1rem] border border-kuartz-line bg-white/72 p-4 text-sm leading-6 text-kuartz-secondary">
        A style direction decision is waiting on the client.
      </p>
      <Link href={`/orders/${pendingApproval?.orderId}`} className="mt-5 inline-flex min-h-10 items-center gap-2 rounded-full bg-kuartz-ink px-4 text-sm font-extrabold text-white transition hover:-translate-y-px hover:bg-kuartz-navy">
...
function ApprovalsPanel({ approvals }: { approvals: PendingApproval[] }) {
  return (
    <section className="rounded-[1.45rem] border border-white/85 bg-white/78 p-5 shadow-[0_22px_65px_rgba(21,22,63,0.07)] backdrop-blur-xl sm:p-6">
      <div className="flex items-end justify-between gap-4">
      ...
      {approvals.length ? (
        <ol className="mt-4 divide-y divide-kuartz-line border-y border-kuartz-line">
          {approvals.slice(0, PANEL_LIMIT).map((row) => (
            <li key={row.fileId} className="py-3">
              <p className="text-sm font-semibold text-kuartz-ink">
                <Link href={`/orders/${row.orderId}`} className="underline-offset-4 hover:underline">{row.orderTitle}</Link>
              </p>
              <p className="mt-1 text-xs text-kuartz-muted">{row.clientName} - sent {dateFormatter.format(row.updatedAt)}</p>
            </li>
          ))}
        </ol>
      ) : (
...
function buildWorkRows({
  ...
  const approvalRows: WorkRow[] = pendingApprovals.slice(0, 2).map((row) => ({
    key: `approval-${row.fileId}`,
    href: `/orders/${row.orderId}`,
    label: "Client decision",
    title: row.orderTitle,
    meta: `${row.clientName} - sent ${dateFormatter.format(row.updatedAt)}`,
    tone: "approval",
  }));
