...
export default async function PendingVendorRatingsPage() {
  ...
  return (
    <div>
      ...
      {groups.length ? (
        <section className="mt-9 divide-y divide-[#d9d8d1] border-y border-[#d9d8d1]">
          {groups.map((group) => (
            <div key={group.orderId} className="grid gap-3 py-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
              ...
              <Link
                ...
                className="inline-flex min-h-[2.75rem] items-center justify-center rounded-[0.8rem] border border-[#171b36] px-4 text-sm font-semibold text-[#171b36] transition-colors duration-200 hover:bg-[#171b36] hover:text-white"
              >
