...
export default async function OrderConfirmationCreatedPage({
  ...
  return (
    <div>
      <Breadcrumbs items={[{ label: "Orders", href: "/orders" }, { label: order.title, href: `/orders/${order.id}` }, { label: "Share confirmation" }]} />
      ...
      {confirmationLink ? (
        <section className="mt-9 grid gap-6 lg:grid-cols-2">
          ...
          <form action={sendOrderConfirmationEmailAction} className="space-y-3 border border-kuartz-line bg-white/55 p-5">
            <input type="hidden" name="orderId" value={order.id} />
            <input type="hidden" name="confirmationId" value={confirmationId} />
            <input type="hidden" name="token" value={token} />
