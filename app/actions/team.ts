...
export async function inviteStaffMemberAction(formData: FormData) {
  ...
  if (error || !data?.user || !tokenHash) redirect("/settings/team?error=The+invitation+could+not+be+sent.&modal=invite");
  ...
  try {
    ...
    redirect("/settings/team?error=The+invitation+email+could+not+be+sent.&modal=invite");
  }
