...
export function InactiveLink({
  ...
  message = "Please ask Kuartz by Roti for a fresh intake link so we can collect your details securely.",
}: {
  title?: string;
  message?: string;
}) {
  return (
    <main className="flex min-h-screen items-center justify-center px-5 py-10">
      <section className="glass-panel w-full max-w-md rounded-[2.25rem] p-8 text-center sm:p-10">
        ...
        <p className="mx-auto mt-4 max-w-sm text-sm leading-6 text-kuartz-muted">{message}</p>
      </section>
    </main>
  );
}