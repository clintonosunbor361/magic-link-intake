  ...
  return (
    <div><Breadcrumbs items={[{ label: "Orders", href: "/orders" }, { label: order.title, href: `/orders/${id}` }, { label: "Invoice" }]} />
      <header className="mt-4 border-b border-kuartz-line pb-8">
        ...
        <Heading className="page-title">{invoice ? invoice.invoiceNumber : "Create the Invoice"}</Heading>
      ...
      {invoice && invoice.totalMinor !== order.finalAgreedPriceMinor ? <p role="alert" className="form-alert mt-6">Invoice total differs from the final agreed price of ₦{formatMinorUnits(order.finalAgreedPriceMinor)}. Review the line items before sending.</p> : null}
      {mismatches.length ? (
          ...
          <aside className="space-y-9">
            {canManage ? (
              <>
                ...
                {invoice.lifecycle !== "void" && canCorrect ? (
                  <div>
