...
export function NewClientForm({
  ...
  async function checkDuplicates(form: HTMLFormElement) {
    ...
    const response = await fetch("/api/clients/duplicate-check", {
      method: "POST",
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4">
          <Link href="/clients" className="text-sm font-semibold text-kuartz-secondary hover:text-kuartz-ink">
            Cancel
          </Link>
          <Button type="submit" disabled={checking}>
            {checking ? "Checking..." : "Create Client"}
          </Button>
        </div>
      </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-kuartz-ink/30 px-4 py-8 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="duplicate-dialog-title">
          <div className="w-full max-w-xl rounded-[1.2rem] border border-kuartz-line bg-[#fbfaf7] p-6 shadow-[0_28px_80px_rgba(24,24,38,0.2)]">
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <h2 id="duplicate-dialog-title" className="mt-1 text-2xl font-extrabold text-kuartz-ink">Possible existing client</h2>
            ...
            <div className="mt-5 space-y-2">
              {modalMatches.map((match) => (
                <div key={match.candidate.id} className="rounded-[0.8rem] border border-kuartz-line bg-white/80 p-3">
                  ...
                  <p className="mt-2 text-xs font-semibold uppercase tracking-wide text-kuartz-danger">
                    {match.reason === "phone" ? "Same phone number" : match.reason === "email" ? "Same email address" : "Similar name"}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" onClick={() => setModalMatches([])}>Go back</Button>
              <Button type="button" onClick={continueWithDuplicate}>Create anyway</Button>
            </div>
          </div>
        </div>
      ) : null}
